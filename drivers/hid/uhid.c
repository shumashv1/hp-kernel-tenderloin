...
static void uhid_queue(struct uhid_device *uhid, struct uhid_event *ev)
{
	...
	if (newhead != uhid->tail) {
		...
		printk(KERN_WARNING "UHID: Output queue is full"); // XXX
		kfree(ev);
	}
}
			   ...
			   const struct uhid_event *ev)
{
	...
	// hid->hid_get_raw_report = uhid_hid_get_raw; // XXX
	...
	if (ret) {
		...
		printk(KERN_ERR "uhid: Cannot register HID device"); // XXX
		goto err_hid;
	}

	printk(KERN_INFO "uhid: Created HID device"); // XXX
